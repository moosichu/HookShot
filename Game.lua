...
function Game:Update(dt)
    ...
    if self.state == Game.STATE_KILL_OCCURED then
        if self.inputs[2].action_down then
            self:ResetLevel()
        end
    elseif self.state == Game.STATE_PLAYING then
...
function Game:Draw()
    ...
    if self.state == Game.STATE_KILL_OCCURED then
        love.graphics.print("Press space to play again!", love.graphics.getWidth() / 2, love.graphics.getHeight() / 2)
    end

    for index, player in pairs(self.player_characters) do
        player:Draw()
        local SCORE_DISTANCE = 40
        love.graphics.draw(player.sprite, 5, 5 + ((index - 1) * SCORE_DISTANCE))
        love.graphics.print("Score: " .. tostring(self.player_scores[index]), 35, 5 + ((index - 1) * SCORE_DISTANCE))
    end
